...
- **Use `flexBasis` for automatic breakpoints**: The `flexBasis` property establishes the wrap breakpoint — the width at which an element triggers a layout break and moves to the next row (e.g. `"300px"`). Assign `flexBasis` to Cards or Charts in a Row so they wrap cleanly at the right point.
