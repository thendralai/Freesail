...
src/
  ...
  standard_catalog.json  # Generated catalog (do not edit manually)
```
...
| Command | Description |
| --- | --- |
| `npx freesail prepare catalog` | Merge schemas and generate `standard_catalog.json` |
...
import { StandardCatalog } from 'standard_catalog';

function App() {
  return (
    <FreesailProvider catalogs={[StandardCatalog]}>
      {/* your app */}
    </FreesailProvider>
  );
}
